...
function linkFn() {
    ...
    // 구체적인요소는 컬렉션[순번] / 컬렉션.item(순번)

    // 3. 각 a요소에 클릭설정하기!
    // a요소 컬렉션 개수만큼 돌면서 클릭이벤트 설정하기
    // for(시;한;증){코드}
    // 시작값은 컬렉션 첫번호인 0 부터 시작함!
    ...
    for (var i = 0; i < alink.length; i++) {
        ...
        alink[i].onclick = function () {
            // 클릭된 a요소 자신 -> this
            // this.innerText는 a요소 글자

            // 1. a요소 텍스트 읽어오기
            var txt = this.innerText;
            console.log(txt);

            // 2. 텍스트 따라 이동할 페이지 분기하기
            // switch case 사용!
            ...
            else if(txt==="페이스북 바로가기" || txt === "인스타그램 바로가기" || txt ==="트위터 바로가기")
